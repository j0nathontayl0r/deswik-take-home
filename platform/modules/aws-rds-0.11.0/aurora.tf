resource "aws_rds_cluster" "aurora_cluster" {
  ...
  db_cluster_parameter_group_name     = var.create_cluster_parameter_group ? aws_rds_cluster_parameter_group.custom_cluster_pg[0].name : null
  apply_immediately                   = var.apply_immediately
  storage_encrypted                   = var.storage_encrypted
  kms_key_id                          = var.kms_key_arn

  lifecycle {
    ignore_changes = [enabled_cloudwatch_logs_exports, master_password]
  }
}
