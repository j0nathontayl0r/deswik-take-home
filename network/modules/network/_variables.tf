...
variable "private_nacl_inbound_tcp_ports" {
  ...
  description = "TCP Ports to allow inbound on private subnet via NACLs (this list cannot be empty)"
}
...
variable "private_nacl_inbound_udp_ports" {
  type        = list(string)
  default     = []
  description = "UDP Ports to allow inbound on private subnet via NACLs (this list cannot be empty)"
}
