...
resource "aws_network_acl_rule" "out_private_to_world_tcp" {
  ...
  rule_action    = "allow"
  cidr_block     = "0.0.0.0/0"
  from_port      = var.private_nacl_outbound_tcp_ports[count.index]
  to_port        = tonumber(var.private_nacl_outbound_tcp_ports[count.index]) == 0 ? "65535" : var.private_nacl_outbound_tcp_ports[count.index]
}
...
resource "aws_network_acl_rule" "out_private_to_world_udp" {
  ...
  rule_action    = "allow"
  cidr_block     = "0.0.0.0/0"
  from_port      = var.private_nacl_outbound_udp_ports[count.index]
  to_port        = tonumber(var.private_nacl_outbound_udp_ports[count.index]) == 0 ? "65535" : var.private_nacl_outbound_udp_ports[count.index]
}
